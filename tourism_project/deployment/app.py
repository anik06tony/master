...
model_path = hf_hub_download(repo_id="Anikettony/Tourism-Package-Prediction", filename="tourism_model.joblib")

# 2. Now load the file using the local path provided by the downloader
...
# Collect user input
CustomerID = st.text_input("CustomerID (customer's ID)")
Age = st.number_input("Age (customer's Age)", min_value=0.0, max_value=100)
...
PitchSatisfactionScore = st.number("Pitch Satisfaction Score", min_value=1, max_value=5,value=3)
